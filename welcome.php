...
include('config.php');
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $sql = "SELECT true FROM GLaDOS WHERE SubjectID = '$myusername' and Password = '$mypassword'";
	  if (mysqli_query($db,$sql)) {
			 $_SESSION['login_user'] = $myusername;
			 header("Location: welcome.php");
	  } else {
		  $sql = "SELECT SubjectID FROM Subjects WHERE SubjectID = '$myusername' and Password = '$mypassword'";
   ...
   <body>
	  ...
	  <?php if ($login_session == 'GLaDOS') {
		...
		<a href="createNewSubjects.php?user=<?php
		  	echo $myusername;
		  ?>"><input type="button" value='Create New Subjects'></a>	  <?php
	  } else {
